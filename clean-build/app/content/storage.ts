...
function ensureRequiredProjects(content: SiteContent): SiteContent {
  ...
  if (defaultEaptekaCard) {
    ...
    for (const [cardId, forcedSlug] of forcedEntries) {
      ...
      } else if (resolvedKey !== forcedSlug && !next.projects[forcedSlug]) {
        if (!(forcedSlug === "vk-miniapps" && resolvedKey === "vkMiniApps")) {
          moveProjectKey(next.projects, resolvedKey, forcedSlug);
        }
      }
