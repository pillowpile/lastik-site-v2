...
function resolveProjectBySlug(projects: Record<string, ProjectPageContent>, slug: string): ProjectPageContent | null {
  ...
  for (const value of Object.values(projects)) {
    ...
    if (folder !== canonicalProjectSlug(slug) && !hero.includes(slugPathNeedle)) {
      continue;
    }
...
export function ProjectPageClient({
  ...
  const useLocalStorageSource =
    process.env.NEXT_PUBLIC_SITE_CONTENT_SOURCE === "local-storage" || process.env.NODE_ENV !== "production" || isLocalBrowser;
  const canUseLocal = isMounted && useLocalStorageSource;
  const resolvedContent = canUseLocal ? resolveProjectBySlug(siteContent.projects, slug) ?? content : content;
