...
import { HERO_BY_FOLDER, THUMB_BY_FOLDER, canonicalProjectSlug, keyToSlug, normalizeAssetPath, normalizeLookup, projectContentScore, sanitizeProjectKey } from "@/app/content/project-helpers";
...
export default function EditorPage() {
  ...
  useEffect(() => {
    ...
    async function loadMaterials() {
      try {
        const response = await fetch(`/api/materials?folder=${encodeURIComponent(selectedMaterialsFolderKey)}`);
        if (!response.ok) {
          if (!cancelled) {
            setProjectMaterials([]);
            setProjectMaterialsFolder(null);
        ...
        const payload = (await response.json()) as { folder: string | null; files: string[]; folders?: string[] };
        if (!cancelled) {
          setProjectMaterials(payload.files ?? []);
          setProjectMaterialsFolder(payload.folder ?? null);
    ...
    async function loadHomeCardMaterials() {
      ...
      await Promise.all(
        cardTargets.map(async ({ cardId, folder }) => {
          if (!byFolder[folder]) {
            try {
              const response = await fetch(`/api/materials?folder=${encodeURIComponent(folder)}`);
              if (!response.ok) {
                byFolder[folder] = [];
              } else {
                const payload = (await response.json()) as { files: string[] };
                byFolder[folder] = payload.files ?? [];
              }
            } catch {
              byFolder[folder] = [];
            }
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  if (fromFolder && toFolder && fromFolder !== toFolder) {
                    try {
                      ...
                      if (!response.ok) {
                        ...
                        const message = payload.error ?? "Failed to rename materials folder";
                        window.alert(message);
                        return;
                      }
                    } catch {
                      window.alert("Failed to rename materials folder");
                      return;
                    }
                  }
                  ...
                  commit((next) => {
                    ...
                    next.projects[toKey].materialsFolder = toFolder || undefined;
                    next.home.projects = next.home.projects.map((card) => {
                      const slug = hrefToProjectSlug(card.href);
          ...
          <div className="editor-actions">
            <button
              ...
              onClick={async () => {
                ...
                try {
                  const response = await fetch("/api/materials?folders=1");
                  const payload = (await response.json()) as { folders?: string[] };
                  existingFolders = new Set((payload.folders ?? []).map((folder) => sanitizeProjectKey(folder)));
                } catch {
                  // fallback to optimistic creation attempts
                }

                for (const key of projectKeys) {
                  const projectData = content.projects[key];
                  const folderKey = sanitizeProjectKey(projectData?.materialsFolder ?? "") || sanitizeProjectKey(key);
                  if (!folderKey || existingFolders.has(folderKey)) {
                    continue;
                  }
                  try {
                    const response = await fetch("/api/materials", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({ projectKey: folderKey }),
                    });
                    if (response.ok) {
              ...
              onClick={async () => {
                ...
                try {
                  await fetch("/api/materials", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ projectKey: nextKey }),
                  });
                } catch {
                  // keep editor flow working even if folder creation fails
                }
